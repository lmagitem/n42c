...
/**
 * Performance test for the NinthMission entity.
 */
class NinthMissionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthMission entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthMissions")
            .get("/api/ninth-missions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthMission")
            .post("/api/ninth-missions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "gameType":"OP"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthMission_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthMission")
                .get("${new_ninthMission_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthMission")
            .delete("${new_ninthMission_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
