...
/**
 * Performance test for the LocalizedNinthStratagem entity.
 */
class LocalizedNinthStratagemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedNinthStratagem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedNinthStratagems")
            .get("/api/localized-ninth-stratagems")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedNinthStratagem")
            .post("/api/localized-ninth-stratagems")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedNinthStratagem")
                .get("${new_localizedNinthStratagem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedNinthStratagem")
            .delete("${new_localizedNinthStratagem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
