...
/**
 * Performance test for the NinthBattle entity.
 */
class NinthBattleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthBattle entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthBattles")
            .get("/api/ninth-battles")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthBattle")
            .post("/api/ninth-battles")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthBattle")
                .get("${new_ninthBattle_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthBattle")
            .delete("${new_ninthBattle_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
