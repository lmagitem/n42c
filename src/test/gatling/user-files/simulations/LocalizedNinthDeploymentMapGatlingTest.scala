...
/**
 * Performance test for the LocalizedNinthDeploymentMap entity.
 */
class LocalizedNinthDeploymentMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedNinthDeploymentMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedNinthDeploymentMaps")
            .get("/api/localized-ninth-deployment-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedNinthDeploymentMap")
            .post("/api/localized-ninth-deployment-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedNinthDeploymentMap")
                .get("${new_localizedNinthDeploymentMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedNinthDeploymentMap")
            .delete("${new_localizedNinthDeploymentMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
