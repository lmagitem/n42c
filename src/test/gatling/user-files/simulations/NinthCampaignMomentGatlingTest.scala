...
/**
 * Performance test for the NinthCampaignMoment entity.
 */
class NinthCampaignMomentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthCampaignMoment entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthCampaignMoments")
            .get("/api/ninth-campaign-moments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthCampaignMoment")
            .post("/api/ninth-campaign-moments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "current":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthCampaignMoment")
                .get("${new_ninthCampaignMoment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthCampaignMoment")
            .delete("${new_ninthCampaignMoment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
