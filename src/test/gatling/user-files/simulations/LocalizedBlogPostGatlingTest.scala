...
/**
 * Performance test for the LocalizedBlogPost entity.
 */
class LocalizedBlogPostGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedBlogPost entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedBlogPosts")
            .get("/api/localized-blog-posts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedBlogPost")
            .post("/api/localized-blog-posts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "title":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_localizedBlogPost_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedBlogPost")
                .get("${new_localizedBlogPost_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedBlogPost")
            .delete("${new_localizedBlogPost_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
