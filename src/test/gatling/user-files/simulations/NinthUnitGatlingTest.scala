...
/**
 * Performance test for the NinthUnit entity.
 */
class NinthUnitGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthUnit entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthUnits")
            .get("/api/ninth-units")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthUnit")
            .post("/api/ninth-units")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthUnit_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthUnit")
                .get("${new_ninthUnit_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthUnit")
            .delete("${new_ninthUnit_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
