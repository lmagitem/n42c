...
/**
 * Performance test for the ProfilePart entity.
 */
class ProfilePartGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProfilePart entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all profileParts")
            .get("/api/profile-parts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new profilePart")
            .post("/api/profile-parts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "title":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_profilePart_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created profilePart")
                .get("${new_profilePart_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created profilePart")
            .delete("${new_profilePart_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
