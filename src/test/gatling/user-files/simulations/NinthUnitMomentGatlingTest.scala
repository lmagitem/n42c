...
/**
 * Performance test for the NinthUnitMoment entity.
 */
class NinthUnitMomentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthUnitMoment entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthUnitMoments")
            .get("/api/ninth-unit-moments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthUnitMoment")
            .post("/api/ninth-unit-moments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "current":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthUnitMoment_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthUnitMoment")
                .get("${new_ninthUnitMoment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthUnitMoment")
            .delete("${new_ninthUnitMoment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
