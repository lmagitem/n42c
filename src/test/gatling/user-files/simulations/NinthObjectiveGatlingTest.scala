...
/**
 * Performance test for the NinthObjective entity.
 */
class NinthObjectiveGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthObjective entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthObjectives")
            .get("/api/ninth-objectives")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthObjective")
            .post("/api/ninth-objectives")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "shareable":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthObjective_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthObjective")
                .get("${new_ninthObjective_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthObjective")
            .delete("${new_ninthObjective_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
