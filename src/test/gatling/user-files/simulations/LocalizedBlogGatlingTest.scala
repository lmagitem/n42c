...
/**
 * Performance test for the LocalizedBlog entity.
 */
class LocalizedBlogGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedBlog entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedBlogs")
            .get("/api/localized-blogs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedBlog")
            .post("/api/localized-blogs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_localizedBlog_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedBlog")
                .get("${new_localizedBlog_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedBlog")
            .delete("${new_localizedBlog_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
