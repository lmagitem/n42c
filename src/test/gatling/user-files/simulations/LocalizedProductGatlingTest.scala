...
/**
 * Performance test for the LocalizedProduct entity.
 */
class LocalizedProductGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedProduct entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedProducts")
            .get("/api/localized-products")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedProduct")
            .post("/api/localized-products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "excerpt":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_localizedProduct_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedProduct")
                .get("${new_localizedProduct_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedProduct")
            .delete("${new_localizedProduct_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
