...
/**
 * Performance test for the NinthArmyUnit entity.
 */
class NinthArmyUnitGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthArmyUnit entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthArmyUnits")
            .get("/api/ninth-army-units")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthArmyUnit")
            .post("/api/ninth-army-units")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "selectableKeywords":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthArmyUnit_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthArmyUnit")
                .get("${new_ninthArmyUnit_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthArmyUnit")
            .delete("${new_ninthArmyUnit_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
