...
/**
 * Performance test for the NinthStratagem entity.
 */
class NinthStratagemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthStratagem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthStratagems")
            .get("/api/ninth-stratagems")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthStratagem")
            .post("/api/ninth-stratagems")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "cost":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthStratagem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthStratagem")
                .get("${new_ninthStratagem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthStratagem")
            .delete("${new_ninthStratagem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
