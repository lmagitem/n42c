...
/**
 * Performance test for the ProfilePartSkill entity.
 */
class ProfilePartSkillGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProfilePartSkill entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all profilePartSkills")
            .get("/api/profile-part-skills")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new profilePartSkill")
            .post("/api/profile-part-skills")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_profilePartSkill_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created profilePartSkill")
                .get("${new_profilePartSkill_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created profilePartSkill")
            .delete("${new_profilePartSkill_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
