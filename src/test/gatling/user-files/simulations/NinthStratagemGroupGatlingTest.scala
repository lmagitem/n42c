...
/**
 * Performance test for the NinthStratagemGroup entity.
 */
class NinthStratagemGroupGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthStratagemGroup entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthStratagemGroups")
            .get("/api/ninth-stratagem-groups")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthStratagemGroup")
            .post("/api/ninth-stratagem-groups")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "shareable":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthStratagemGroup_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthStratagemGroup")
                .get("${new_ninthStratagemGroup_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthStratagemGroup")
            .delete("${new_ninthStratagemGroup_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
