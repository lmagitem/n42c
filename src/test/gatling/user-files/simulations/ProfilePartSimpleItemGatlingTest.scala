...
/**
 * Performance test for the ProfilePartSimpleItem entity.
 */
class ProfilePartSimpleItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProfilePartSimpleItem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all profilePartSimpleItems")
            .get("/api/profile-part-simple-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new profilePartSimpleItem")
            .post("/api/profile-part-simple-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "title":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_profilePartSimpleItem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created profilePartSimpleItem")
                .get("${new_profilePartSimpleItem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created profilePartSimpleItem")
            .delete("${new_profilePartSimpleItem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
