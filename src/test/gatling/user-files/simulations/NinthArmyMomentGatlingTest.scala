...
/**
 * Performance test for the NinthArmyMoment entity.
 */
class NinthArmyMomentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthArmyMoment entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthArmyMoments")
            .get("/api/ninth-army-moments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthArmyMoment")
            .post("/api/ninth-army-moments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "current":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthArmyMoment")
                .get("${new_ninthArmyMoment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthArmyMoment")
            .delete("${new_ninthArmyMoment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
