...
/**
 * Performance test for the AppUserProfile entity.
 */
class AppUserProfileGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AppUserProfile entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all appUserProfiles")
            .get("/api/app-user-profiles")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new appUserProfile")
            .post("/api/app-user-profiles")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_appUserProfile_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created appUserProfile")
                .get("${new_appUserProfile_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created appUserProfile")
            .delete("${new_appUserProfile_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
