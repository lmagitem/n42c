...
/**
 * Performance test for the NinthMissionRule entity.
 */
class NinthMissionRuleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthMissionRule entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthMissionRules")
            .get("/api/ninth-mission-rules")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthMissionRule")
            .post("/api/ninth-mission-rules")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthMissionRule_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthMissionRule")
                .get("${new_ninthMissionRule_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthMissionRule")
            .delete("${new_ninthMissionRule_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
