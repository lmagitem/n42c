...
/**
 * Performance test for the NinthCampaign entity.
 */
class NinthCampaignGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthCampaign entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthCampaigns")
            .get("/api/ninth-campaigns")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthCampaign")
            .post("/api/ninth-campaigns")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthCampaign")
                .get("${new_ninthCampaign_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthCampaign")
            .delete("${new_ninthCampaign_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
