...
/**
 * Performance test for the LocalizedNinthMission entity.
 */
class LocalizedNinthMissionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedNinthMission entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedNinthMissions")
            .get("/api/localized-ninth-missions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedNinthMission")
            .post("/api/localized-ninth-missions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedNinthMission")
                .get("${new_localizedNinthMission_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedNinthMission")
            .delete("${new_localizedNinthMission_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
