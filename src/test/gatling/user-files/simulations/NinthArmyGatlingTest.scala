...
/**
 * Performance test for the NinthArmy entity.
 */
class NinthArmyGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthArmy entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthArmies")
            .get("/api/ninth-armies")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthArmy")
            .post("/api/ninth-armies")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthArmy_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthArmy")
                .get("${new_ninthArmy_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthArmy")
            .delete("${new_ninthArmy_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
