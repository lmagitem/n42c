...
/**
 * Performance test for the LocalizedNinthObjective entity.
 */
class LocalizedNinthObjectiveGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LocalizedNinthObjective entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localizedNinthObjectives")
            .get("/api/localized-ninth-objectives")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localizedNinthObjective")
            .post("/api/localized-ninth-objectives")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created localizedNinthObjective")
                .get("${new_localizedNinthObjective_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localizedNinthObjective")
            .delete("${new_localizedNinthObjective_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
