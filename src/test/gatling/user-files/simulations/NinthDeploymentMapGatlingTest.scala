...
/**
 * Performance test for the NinthDeploymentMap entity.
 */
class NinthDeploymentMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NinthDeploymentMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ninthDeploymentMaps")
            .get("/api/ninth-deployment-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ninthDeploymentMap")
            .post("/api/ninth-deployment-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "url":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ninthDeploymentMap_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ninthDeploymentMap")
                .get("${new_ninthDeploymentMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ninthDeploymentMap")
            .delete("${new_ninthDeploymentMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
