...
export class NinthArmyMomentService {
  ...
  create(ninthArmyMoment: INinthArmyMoment): Observable<EntityResponseType> {
    return this.http.post<INinthArmyMoment>(this.resourceUrl, ninthArmyMoment, { observe: 'response' });
  }

  update(ninthArmyMoment: INinthArmyMoment): Observable<EntityResponseType> {
    return this.http.put<INinthArmyMoment>(this.resourceUrl, ninthArmyMoment, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<INinthArmyMoment>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<INinthArmyMoment[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
